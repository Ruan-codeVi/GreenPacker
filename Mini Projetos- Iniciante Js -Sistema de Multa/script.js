function verificar(){
    ...
    if(veloAtual.value.length <=0){
        ...
        if(veloc<=80 && cintOk[0].checked && cnhOk[0].checked){
            ...
            res.innerHTML =`<strong>Parabéns nenhuma infração de trânsito.</strong> <p>Diraja com cuidado e sempre obdeça as leis de trânsito</p>`
        }
        if(veloc > 80){
            res.style.color = 'red';
            ...
            res.innerHTML = `<p><strong>Multado!</strong></p><p> *Excesso de Velocidade</p>`
             if(veloc > 80 && cintOk[1].checked){
                 img.setAttribute('src','guincho.png');
